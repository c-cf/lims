...
const CancelRequestModal=({requestId,onClose,onCancelled,showToast})=>{const[reason,setReason]=React.useState('');const[busy,setBusy]=React.useState(false);const[err,setErr]=React.useState(null);const canConfirm=reason.trim().length>0&&!busy;const confirm=async()=>{setBusy(true);setErr(null);try{await api.requests.cancel(requestId,reason.trim());showToast?.(`Request #${requestId} cancelled`);onCancelled?.();}catch(e){setErr(e.message||String(e));}finally{setBusy(false);}};return<div style={{position:'fixed',inset:0,zIndex:200,background:'rgba(20,20,28,0.42)',backdropFilter:'blur(2px)',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}onClick={onClose}>
