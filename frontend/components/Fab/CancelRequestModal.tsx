...
const CancelRequestModal = ({ requestId, onClose, onCancelled, showToast }) => {
  ...
  const confirm = async () => {
    ...
    setErr(null);
    try {
      await api.requests.cancel(requestId, reason.trim());
    ...
    >
      <div
        ...
        style={{
          ...
          background: '#fff',
          borderRadius: 14,
          boxShadow: '0 24px 60px rgba(20,20,28,0.32)',
          padding: 24,
        }}
      >
        <div
          style={{ fontSize: 17, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}
        >
        ...
        <div style={{ marginTop: 18, display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <PrimaryBtn
            disabled={!canConfirm}
            onClick={confirm}
            style={{ background: canConfirm ? '#c0394a' : '#cbcbd6' }}
          >
