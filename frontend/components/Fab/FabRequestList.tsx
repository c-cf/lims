...
const FabRequestList = ({
  ...
  const counts = React.useMemo(
    () => Object.fromEntries(TABS.map((t) => [t.id, requests.filter(t.filter).length])),
    [requests],
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 14 }}>
        Showing <strong style={{ color: 'var(--text-primary)' }}>{list.length}</strong> of{' '}
        {baseList.length} requests
      </div>
                ...
                {drafts ? (
                  <>
                    ...
                    <span
                      ...
                      onMouseEnter={(e) => {
                        if (deletingId !== r.id) e.currentTarget.style.background = '#fde4e4';
                      }}
                      onMouseLeave={(e) => {
