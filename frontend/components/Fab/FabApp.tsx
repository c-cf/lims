...
const FabApp = ({ route, navigate }: { route: Route; navigate: Navigate }) => {
  ...
  else if (route.page === 'fab_requests')
    page = <FabRequestList navigate={navigate} initialTab={route.tab || 'all'} />;
  else if (route.page === 'fab_drafts')
    page = <FabRequestList navigate={navigate} drafts titleOverride="Drafts" />;
  else if (route.page === 'fab_new')
    page = <FabNewRequest navigate={navigate} showToast={showToast} />;
  else if (route.page === 'fab_draft_edit')
    page = <FabDraftEdit id={route.id} navigate={navigate} showToast={showToast} />;
  else if (route.page === 'fab_request')
    page = <FabRequestDetail id={route.id} navigate={navigate} showToast={showToast} />;
