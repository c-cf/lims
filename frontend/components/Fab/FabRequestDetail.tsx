...
import useSampleExperimentsForRequest from '@/components/Fab/hooks/useSampleExperimentsForRequest';
...
const FabRequestDetail = ({
  ...
  const onShip = async () => {
    ...
    setShipBusy(true);
    try {
      await api.requests.ship(r.id);
      showToast?.('Wafers shipped');
      refresh();
    } catch (e) {
      showToast?.(`Ship failed: ${e.message || e}`);
    } finally {
      setShipBusy(false);
    }
  };
  ...
  if (error || !r) {
    return (
      <FabPage
        breadcrumb={
          <button
            onClick={() => navigate({ page: 'fab_requests' })}
            style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: 4,
          ...
          >
            <F.ChevronLeft size={14} /> My Requests
          </button>
        }
          ...
          {r.status !== 'draft' &&
            ...
            overallIdx >= 0 && (
              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                Currently at{' '}
                <strong style={{ color: 'var(--text-primary)' }}>{WAFER_PHASES[overallIdx]}</strong>
              </span>
            )}
        </span>
      }
          ...
          {r.rawStatus === 'approved' && (
            <button
              ...
              style={{
                padding: '9px 15px',
                borderRadius: 8,
                background: shipBusy ? '#cbcbd6' : '#6c67b8',
                color: '#fff',
                fontWeight: 600,
                fontSize: 13,
            ...
            >
              <F.Package size={14} strokeWidth={2.5} /> {shipBusy ? 'Shipping…' : 'Ship Wafers'}
            </button>
          )}
          {canCancel && (
            <button
            ...
            >
              <F.X size={14} strokeWidth={2.5} /> Cancel Request
            </button>
          )}
        </span>
      }
      ...
      <FabCard padding={0} style={{ marginBottom: 18 }}>
        ...
        {r.note && (
          <div
            style={{
              padding: '16px 24px',
              borderBottom: r.history.length > 0 ? '1px solid rgba(0,0,0,0.06)' : 'none',
            }}
          >
            <div
              style={{
      ...
      <FabCard padding={0} style={{ marginBottom: 18 }}>
        <PlainCardHeader
          right={
            ...
            >
              {r.samples.length} wafer{r.samples.length === 1 ? '' : 's'}
            </span>
          }
        >
          ...
          right={
            ...
            >
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                <span style={{ width: 9, height: 9, borderRadius: 999, background: '#157a4a' }} />
                Pass
              </span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                <span style={{ width: 9, height: 9, borderRadius: 999, background: '#a93445' }} />
                Fail
              </span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                <span
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div
                      style={{
                        ...
                        fontSize: 18,
                        fontWeight: 700,
                        color: doneCount === total && total > 0 ? '#157a4a' : '#1e1e24',
                        letterSpacing: '-0.01em',
                      }}
                    >
                      {doneCount}
