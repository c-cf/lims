...
const InProgressRow = ({
  ...
  React.useEffect(() => {
    if (!open || detail || detailLoading || !api?.requests) return;
  ...
  return (
    <div style={{ borderTop: '1px solid #f5f5f9' }}>
      ...
      >
        ...
        >
          {request.submitted ? request.submitted.split(' ')[0] : '—'}
        </span>
        <F.ChevronDown
          size={15}
          color="#a8a8b8"
        ...
        >
          ...
          <div style={{ marginTop: 14, display: 'flex', justifyContent: 'flex-end' }}>
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigate({ page: 'fab_request', id: request.id });
              }}
              style={{
                fontSize: 13,
                fontWeight: 600,
            ...
            >
              Open request <F.ArrowRight size={13} />
            </button>
          </div>
        </div>
      )}
