...
const stepFromRequest = (r: RequestRow) => {
  ...
  if (samples.length > 0) {
    ...
    const anyProcessing = samples.some(
      (s) => s.status === 'processing' || s.status === 'in_wip' || s.status === 'completed',
    );
    if (anyProcessing) return { idx: 2 };
    const anyShipped = samples.some(
      (s) => s.raw_status === 'shipped' || s.status === 'received' || s.raw_status === 'received',
    );
