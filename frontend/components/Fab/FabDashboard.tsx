...
const FabDashboard = ({ navigate }) => {
  ...
  const activity = React.useMemo(() => {
    ...
    requests.forEach((r) => {
      ...
      if (!at) return;
      const rs = r.rawStatus;
      if (rs === 'approved') items.push({ action: 'APPROVE', at, request: r });
      else if (rs === 'sample_shipped') items.push({ action: 'SHIP', at, request: r });
      else if (rs === 'in_progress') items.push({ action: 'IN_PROG', at, request: r });
      else if (rs === 'completed') items.push({ action: 'COMPLETED', at, request: r });
      else if (rs === 'returned') items.push({ action: 'RETURN', at, request: r });
      else if (rs === 'rejected') items.push({ action: 'REJECT', at, request: r });
    });
  ...
  if (loading && requests.length === 0) {
    return (
      <FabPage
        ...
        right={
          <PrimaryBtn icon={<F.Plus size={16} />} onClick={() => navigate({ page: 'fab_new' })}>
            New Request
          </PrimaryBtn>
        }
      >
    ...
    >
      ...
      <FabCard padding={0} style={{ marginBottom: 18, overflow: 'hidden' }}>
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: '80px 1fr 130px 130px 24px',
            padding: '10px 24px',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 18 }}>
          ...
          <div>
            {attention.length === 0 ? (
              ...
              attention.map((r) => (
                <button
                  key={r.id}
                  onClick={() => navigate({ page: 'fab_request', id: r.id })}
                  style={{
                    width: '100%',
                    textAlign: 'left',
                    display: 'flex',
        ...
        <FabCard padding={0} style={{ overflow: 'hidden' }}>
          <BannerHeader
            ...
            accentLight="#c7dde6"
            right={
              <HeaderLinkButton accent="#9ebbc8" onClick={() => navigate({ page: 'fab_drafts' })}>
                Open Drafts <F.ArrowRight size={13} />
              </HeaderLinkButton>
            }
          />
          <div>
            {drafts.length === 0 ? (
              ...
              >
                ...
                <div style={{ marginTop: 10 }}>
                  ...
                  >
                    <F.Plus size={13} /> Start a new request
                  </button>
                </div>
              </div>
            ) : (
              drafts.slice(0, 4).map((r) => (
                <button
                  key={r.id}
                  onClick={() => navigate({ page: 'fab_draft_edit', id: r.id })}
                  style={{
                    width: '100%',
                    textAlign: 'left',
                    display: 'flex',
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      #{r.id} · {r.sampleCount ?? r.samples.length} wafer
                      {(r.sampleCount ?? r.samples.length) === 1 ? '' : 's'} ·{' '}
                      {r.created.split(' ')[0]}
                    </div>
                  </div>
                  <span style={{ fontSize: 12.5, fontWeight: 600, color: '#6c67b8' }}>
                    Continue →
                  </span>
                </button>
              ))
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        gap: 12,
                        marginBottom: di === days.length - 1 ? 0 : 22,
                      }}
                    >
                      {items.map((a, i) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              position: 'relative',
                              display: 'grid',
                              gridTemplateColumns: '60px 36px 1fr',
                              alignItems: 'center',
                              gap: 16,
                            }}
                          >
