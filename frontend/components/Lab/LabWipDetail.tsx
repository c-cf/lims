...
const LF=I;
const LabWipDetail=({id,navigate,showToast})=>{const{wip:w,loading,error,refresh}=useLabWipDetail(id);const[busy,setBusy]=React.useState(false);const[actionError,setActionError]=React.useState(null);const runAction=async(op,label)=>{setBusy(true);setActionError(null);try{await op();showToast?.(label);refresh();}catch(e){setActionError(e.message||String(e));}finally{setBusy(false);}};const onAbort=()=>{if(!w)return;if(!window.confirm(`Abort ${w.code}? This cannot be undone.`))return;runAction(()=>api.wips.abort(w.id),`${w.code} aborted`);};const[addDispatchOpen,setAddDispatchOpen]=React.useState(false);const onAddDispatch=()=>setAddDispatchOpen(true);const onDispatchCreated=()=>{setAddDispatchOpen(false);showToast?.('Dispatch created');refresh();};if(loading&&!w){return<Page title="Loading WIP…">
