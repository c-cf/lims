...
import useLabWipDetail from '@/components/Lab/hooks/useLabWipDetail';
...
const LabWipDetail = ({ id, navigate, showToast }) => {
  ...
  const onAbort = () => {
    ...
    runAction(() => api.wips.abort(w.id), `${w.code} aborted`);
  };
  const [addDispatchOpen, setAddDispatchOpen] = React.useState(false);
  ...
  if (error || !w) {
    return (
      <Page
        breadcrumb={
          <Breadcrumb
            items={[{ label: 'WIP', onClick: () => navigate({ page: 'lab_wip' }) }, { label: '?' }]}
          />
        }
        title="WIP not found"
      >
      ...
      subtitle={
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <span style={{ color: muted, fontSize: 13 }}>
            · {w.sampleCount} sample{w.sampleCount === 1 ? '' : 's'}
          </span>
          {w.created && (
            <span style={{ color: muted, fontSize: 13 }}>· created {w.created.split(' ')[0]}</span>
          )}
        </span>
      }
      right={
        isActive &&
        (hasActiveDispatch ? (
          <SecondaryBtn danger onClick={onAbort} disabled={busy}>
            {busy ? '…' : 'Abort WIP'}
          </SecondaryBtn>
        ) : (
          <PrimaryBtn icon={<LF.Plus size={14} />} onClick={onAddDispatch} disabled={busy}>
            Create Dispatch
          </PrimaryBtn>
        ))
      }
        ...
        return (
          ...
          >
            <span>
              ...
              />
              {exceptionDispatch.code} has an execution exception. Abort or redispatch it to
              continue.
            </span>
            <button
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <Card padding={0}>
            ...
            {w.dispatches.length === 0 ? (
              <div
                style={{ padding: '28px 20px', textAlign: 'center', color: muted, fontSize: 13 }}
              >
                No dispatches yet
                {isActive && !hasActiveDispatch ? ' — use Create Dispatch above to start one' : ''}
              </div>
            ) : (
              <>
                <div
          ...
          {w.note && (
            <Card padding={0}>
              ...
              <div style={{ padding: 22, fontSize: 14, color: ink, lineHeight: 1.55 }}>
                {w.note}
              </div>
            </Card>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <Card padding={0}>
            ...
            <div>
              {w.samples.length === 0 ? (
                <div style={{ padding: '20px 22px', color: muted, fontSize: 13 }}>
