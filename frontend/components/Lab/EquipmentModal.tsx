...
const EquipmentModal = ({
  ...
  const [capIds, setCapIds] = React.useState([]);
  ...
  React.useEffect(() => {
    ...
    if (initial) {
      ...
      try {
        setParamsJson(JSON.stringify(initial.parameters || {}, null, 2) || '{}');
      } catch {
        setParamsJson('{}');
      }
    } else {
  ...
  const submit = async () => {
    ...
    try {
      if (isEdit) {
        await api.equipment.update(initial.id, {
          name: name.trim(),
        ...
        if (capsChanged) {
          await api.equipment.setCapabilities(initial.id, capIds);
        }
      } else {
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          ...
          >
            {typesLoading ? (
              <div style={{ padding: 14, color: muted, fontSize: 13, textAlign: 'center' }}>
                Loading…
              </div>
            ) : experimentTypes.length === 0 ? (
              <div style={{ padding: 14, color: muted, fontSize: 13, textAlign: 'center' }}>
                No experiment types defined yet.
              </div>
            ) : (
              experimentTypes.map((t) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={capIds.includes(t.id)}
                    onChange={() => toggleCap(t.id)}
          ...
          <div style={{ fontSize: 12, color: muted, marginTop: 6 }}>
            Experiment types this unit can run.{' '}
            {isEdit && capsChanged
              ? 'Changes will save via a separate request after the equipment update.'
              : ''}
          </div>
        </div>
        <div>
