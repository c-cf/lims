...
const LabDashboard = ({ navigate }) => {
  ...
  const needsRecord = liveDispatches.filter(
    (d) => d.status === 'unloaded' || d.status === 'exception',
  ).length;
  const activeDispatches = liveDispatches
    .filter((d) => d.status === 'running' || d.status === 'pending' || d.status === 'dispatched')
    .slice(0, 5);
  const toRecord = liveDispatches.filter(
    (d) => d.status === 'unloaded' || d.status === 'exception',
  );
  ...
  const cachedUser = api && api.auth && api.auth.cachedUser ? api.auth.cachedUser() : null;
  ...
  const tiles = [
    {
      ...
      value: v(incoming),
      onClick: () => navigate({ page: 'lab_samples', tab: 'incoming' }),
      icon: <LF.Inbox size={16} color="#a06618" />,
      tint: '#fef4dd',
    },
    {
      label: 'Active WIPs',
      ...
      icon: <LF.WIP size={16} color="#4f4a8f" />,
      tint: '#ecebf3',
    },
    {
      label: 'Dispatches live',
      value: v(runningDps),
      onClick: () => navigate({ page: 'lab_dispatches', tab: 'active' }),
      icon: <LF.Activity size={16} color="#a93445" />,
      tint: '#fbe4e6',
    },
    {
      label: 'To record',
      value: v(needsRecord),
      onClick: () => navigate({ page: 'lab_dispatches', tab: 'record' }),
      icon: <LF.ClipboardList size={16} color="#2e6a47" />,
      tint: '#e7f0e9',
    },
  ];
  return (
    <Page title="Dashboard" subtitle={`Welcome back, ${subtitleName} · ${subtitleDate}`}>
      {countsError && (
        <div
          style={{
            padding: '12px 16px',
          ...
          <Card padding={0}>
            ...
            <div>
              {activeDispatches.length === 0 && (
                <div
                  style={{ padding: '28px 22px', textAlign: 'center', color: muted, fontSize: 13 }}
                >
              ...
              {activeDispatches.map((d) => {
                ...
                if (d.status === 'running' && d.dispatchedAtIso && totalSec > 0) {
                  ...
                  remainLabel = `${UI.formatDuration(Math.ceil(Math.max(0, totalSec - elapsedSec)))} left`;
                }
                return (
                  <button
                    key={d.id}
                  ...
                  >
                    ...
                    {d.status === 'running' && totalSec > 0 && (
                      <div>
                        ...
                        <div
                          style={{ fontSize: 11, color: accent, fontFamily: 'var(--font-mono)' }}
                        >
                          {remainLabel}
                        </div>
                      </div>
          ...
          {toRecord.length > 0 && (
            ...
            >
              <CardHeader
                style={{
                  background:
                    'linear-gradient(90deg, rgba(244,168,191,0.12), rgba(187,183,232,0.12))',
              ...
              {toRecord.map((d) => (
                <button
                  ...
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '90px 1fr 130px auto',
                    alignItems: 'center',
                    gap: 12,
                    width: '100%',
                    padding: '13px 22px',
                ...
                >
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12.5, color: text2 }}>
                    {d.code}
                  </span>
        ...
        <Card padding={0}>
          ...
          <div>
            {liveEquipment.length === 0 && (
              <div
                style={{ padding: '24px 20px', textAlign: 'center', color: muted, fontSize: 13 }}
              >
