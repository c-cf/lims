...
import useLabWips from '@/components/Lab/hooks/useLabWips';
...
const LabWipList = ({ navigate, showToast }) => {
  ...
  return (
    <Page
      ...
      right={
        <PrimaryBtn icon={<LF.Plus size={14} />} onClick={openModal}>
          New WIP
        </PrimaryBtn>
      }
    >
      ...
      <div style={{ fontSize: 13, color: muted, marginBottom: 14 }}>
        Showing <strong style={{ color: ink }}>{filtered.length}</strong> of {wips.length} WIP
        {wips.length === 1 ? '' : 's'}
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((w) => {
            ...
            return (
              <button
                key={w.id}
                onClick={() => navigate({ page: 'lab_wip_detail', id: w.id })}
                style={{
                  display: 'grid',
