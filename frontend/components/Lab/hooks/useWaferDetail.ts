    ...
    (async () => {
      try {
        ...
        const [request, experiments] = await Promise.all([
          api.requests.get(sample.requestId).catch(() => null),
          api.samples.getExperiments(sample.id).catch(() => []),
        ]);
        let wip = null;
        if (sample.hasWip) {
          const wipList = await api.wips.list({ status: 'in_progress' }).catch(() => []);
          for (const row of wipList) {
            if (cancelled) return;
            const detail = await api.wips.get(row.id).catch(() => null);
