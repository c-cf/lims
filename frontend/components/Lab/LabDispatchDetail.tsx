...
const LabDispatchDetail = ({
  ...
  else if (d.status === 'exception' && d.raw_status === 'execution_exception')
    actions = (
      <>
        <SecondaryBtn
          danger
      ...
      <Card padding={0} style={{ marginBottom: 18 }}>
              ...
              <React.Fragment key={s}>
                ...
                >
                  ...
                  >
                    ...
                    {cur && (
                      <span
                        style={{ width: 8, height: 8, borderRadius: 999, background: '#fff' }}
                      />
                    )}
                  </span>
                  <span
        ...
        {(d.status === 'aborted' || d.status === 'exception') && (
          ...
          >
            ...
            {d.status === 'aborted'
              ? 'Dispatch aborted before completion.'
              : d.raw_status === 'pending_redispatch'
                ? 'Dispatch superseded — a new dispatch has been created to continue this WIP.'
                : 'Execution exception — abort this dispatch to open a new one, or redispatch on the same equipment.'}
          </div>
        )}
      </Card>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {waferResults.map((w) => {
                        const v = w.verdict;
                        const pillBg =
                          v === 'pass' ? '#e7f0e9' : v === 'fail' ? '#fbe4e6' : '#f1f1f5';
