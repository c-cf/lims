...
const LF=I;
const LabWaferDetail=({id,navigate,showToast})=>{const{data,loading,error,refresh}=useWaferDetail(id);const{data:expTypes}=useLabExperimentTypes();const[busy,setBusy]=React.useState(false);const[actionError,setActionError]=React.useState(null);const runAction=async(op,label)=>{setBusy(true);setActionError(null);try{await op();showToast?.(label);refresh();}catch(e){setActionError(e.message||String(e));}finally{setBusy(false);}};if(loading&&!data){return<Page title="Loading wafer…">
