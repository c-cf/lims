    ...
    <Page
      ...
      subtitle={
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontFamily: 'var(--font-mono)', color: muted }}>
            Request #{String(w.requestId).padStart(4, '0')}
          </span>
          <Pill kind={w.status} />
          <Pill kind={urgency} />
        </span>
      ...
      right={
        w.status === 'incoming' && (
          <>
            <SecondaryBtn danger disabled={busy} onClick={onReject} icon={<LF.X size={14} />}>
              {busy ? '…' : 'Reject'}
            </SecondaryBtn>
            <PrimaryBtn disabled={busy} onClick={onReceive} icon={<LF.Check size={14} />}>
              {busy ? '…' : 'Receive'}
            </PrimaryBtn>
          </>
        )
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <Card padding={0}>
            ...
            <div
              style={{ padding: 22, display: 'grid', gridTemplateColumns: '120px 1fr', rowGap: 12 }}
            >
          ...
          {wip && (
            <Card padding={0}>
              ...
              <button
                onClick={() => navigate({ page: 'lab_wip_detail', id: wip.id })}
                style={{
                  width: '100%',
                  textAlign: 'left',
                  background: '#fff',
        ...
        <Card padding={22}>
          ...
          {[
            ...
            return (
              <div
                key={s.k}
                style={{ display: 'flex', gap: 10, paddingBottom: 12, position: 'relative' }}
              >
                  ...
                  {i < arr.length - 1 && (
                    <div
                      style={{
                        flex: 1,
                        width: 2,
                        background: reached && i < cur ? accent : '#ececf2',
                        marginTop: 2,
                      }}
                    />
                  )}
          ...
          {w.status === 'rejected' && (
            ...
            >
              <strong>Rejected.</strong> Status set during receiving; see request detail for the
              reason.
            </div>
          )}
        </Card>
