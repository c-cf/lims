...
const LabDispatchList = ({ navigate, defaultTab = 'active' }) => {
  ...
  return (
    <Page
      title="Dispatches"
      subtitle="One experiment run on one piece of equipment, derived from a WIP"
    >
      {error && (
        <div
          style={{
      ...
      <div style={{ fontSize: 13, color: muted, marginBottom: 14 }}>
        Showing <strong style={{ color: ink }}>{filtered.length}</strong> of {dispatches.length}{' '}
        dispatch{dispatches.length === 1 ? '' : 'es'}
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((d) => {
            ...
            if (d.status === 'running' && d.dispatchedAtIso && totalSec > 0) {
              ...
              remainLabel = `${UI.formatDuration(Math.ceil(remainSec))} left`;
              showBar = true;
            }
            const expCode =
              findExp(d.experimentId)?.code ||
              (d.experimentName
                ? d.experimentName
                    .split(/\s+/)
                    .map((t) => t[0])
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div
                    style={{ minWidth: 0, display: 'inline-flex', alignItems: 'center', gap: 10 }}
                  >
                    <span
                      style={{
                        fontSize: 10.5,
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      >{`WIP-${String(d.wipId).padStart(4, '0')}`}</div>
                    </div>
                  </div>
                  <span
                    style={{
                  ...
                  <span
                    style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: text2 }}
                ...
                {d.status === 'running' && d.dispatchedAt && (
                  <div
                    style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${lineSoft}` }}
                  >
                    <div
                      style={{
                        display: 'flex',
                    ...
                    >
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <span style={{ color: muted }}>
                          est. {UI.formatDuration(d.estimatedDurationSeconds)}
                        </span>
                      </span>
