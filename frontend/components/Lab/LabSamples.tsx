...
const LabSamples = ({
  ...
  const tabs = [
    { id: 'all', label: 'All', count: wafers.length },
    {
      id: 'incoming',
      label: 'Incoming',
      count: wafers.filter((w) => w.status === 'incoming').length,
    },
    {
      id: 'received',
      label: 'Received',
      count: wafers.filter((w) => w.status === 'received').length,
    },
    { id: 'in_wip', label: 'In WIP', count: wafers.filter((w) => w.status === 'in_wip').length },
    {
      id: 'completed',
      label: 'Completed',
      count: wafers.filter((w) => w.status === 'completed').length,
    },
    {
      id: 'rejected',
      label: 'Rejected',
      count: wafers.filter((w) => w.status === 'rejected').length,
    },
  ];
  ...
  return (
    <Page
      ...
      right={
        <SecondaryBtn icon={<LF.Inbox size={14} />} onClick={handleBulkReceive}>
          Bulk receive incoming
        </SecondaryBtn>
      }
    >
      {(error || actionError) && (
      ...
      <div style={{ fontSize: 13, color: muted, marginBottom: 14 }}>
        Showing <strong style={{ color: ink }}>{list.length}</strong> of {wafers.length} wafer
        {wafers.length === 1 ? '' : 's'}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {list.length === 0 ? (
          <Card padding={48} style={{ textAlign: 'center', color: muted }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: text2 }}>
              No wafers in this view
            </div>
          </Card>
        ) : (
          list.map((w) => {
            ...
            return (
              <button
                key={w.id}
                onClick={() => navigate({ page: 'lab_wafer', id: w.id })}
                style={{
                  display: 'grid',
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor =
                    fmt.level === 'overdue' ? '#e88a93' : 'rgba(0,0,0,0.18)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor =
                    fmt.level === 'overdue' ? '#f1b9c0' : 'rgba(0,0,0,0.08)';
                }}
              >
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  {showDot && (
                    <span
                      title="Past or near deadline"
                      style={{
                        width: 6,
                        height: 6,
                        borderRadius: 999,
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: ink }}>
                    ...
                    <span style={{ color: muted, fontWeight: 500 }}>
                      · #{String(w.requestId).padStart(4, '0')}
                    </span>
                  </div>
                  <div
                    style={{
