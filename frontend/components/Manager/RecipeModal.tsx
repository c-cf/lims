...
const RecipeModal = ({
  ...
  const schema = React.useMemo(() => (slug ? RECIPE_PARAM_SCHEMA[slug] || [] : []), [slug]);
  React.useEffect(() => {
    if (!open) return;
    setErr(null);
    setBusy(false);
    if (initial) {
      ...
      try {
        setParamsJson(JSON.stringify(incomingParams, null, 2) || '{}');
      } catch {
        setParamsJson('{}');
      }
    } else {
  ...
  React.useEffect(() => {
    ...
    setParamsKv((prev) => {
      const next = { ...prev };
      schema.forEach((s) => {
        if (next[s.key] == null) next[s.key] = '';
      });
      return next;
    });
  }, [open, slug, schema]);
  const valid =
    name.trim().length > 0 && name.trim().length <= 200 && (isEdit || !!experimentTypeId);
  const submit = async () => {
    setBusy(true);
    setErr(null);
    let parameters;
    if (schema.length > 0) {
      ...
      if (!trimmed) {
        ...
        try {
          ...
          setErr('Parameters must be valid JSON.');
          setBusy(false);
          return;
        }
        if (parameters === null || typeof parameters !== 'object' || Array.isArray(parameters)) {
          setErr('Parameters must be a JSON object.');
          setBusy(false);
          return;
        }
      }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          ...
          <TextInput
            ...
            placeholder="e.g. TCT_Standard_Reflow_Simulation_v1"
          />
        </div>
        <div>
          ...
          {isEdit ? (
            ...
            >
              ...
              {!typesLoading && experimentTypes.length === 0 && (
                <option value="">No experiment types</option>
              )}
        ...
        <div>
          ...
          <TextArea
            ...
            placeholder="When this recipe is used and why."
          />
        </div>
        <div>
          ...
          {schema.length > 0 ? (
            ...
            <>
              ...
              />
              <div style={{ fontSize: 12, color: mMuted, marginTop: 6 }}>
                No schema defined for {activeExpName || 'this experiment type'} — enter a JSON
                object. Leave as <code>{'{}'}</code> for none.
              </div>
            </>
          )}
        </div>
