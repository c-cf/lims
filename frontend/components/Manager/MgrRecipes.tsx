...
const MgrRecipes = ({ showToast }) => {
  ...
  const onDelete = async (rec) => {
    ...
    setDeleteError(null);
    try {
      await api.recipes.remove(rec.id);
  ...
  return (
    <Page
      ...
      right={
        <PrimaryBtn icon={<MI.Plus size={14} />} onClick={openNew}>
          New Recipe
        </PrimaryBtn>
      }
    >
          ...
          recipes.map((rec) => {
            const expCode = rec.experimentName
              ? rec.experimentName
                  ...
                  .toUpperCase()
              : '—';
            const expName = rec.experimentName || '—';
              ...
              >
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {paramEntries.slice(0, 4).map(([k, v]) => (
                    ...
                    >
                      {k}{' '}
                      <strong style={{ color: mInk }}>
                        {typeof v === 'object' ? JSON.stringify(v) : String(v)}
                      </strong>
                    </span>
                  ))}
                  {paramEntries.length > 4 && (
                    <span style={{ fontSize: 11.5, color: mMuted, alignSelf: 'center' }}>
                      +{paramEntries.length - 4} more
                    </span>
                  )}
                  {paramEntries.length === 0 && (
                    <span style={{ fontSize: 12, color: mMuted, fontStyle: 'italic' }}>
                      No parameters
                    </span>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                  ...
                  >
                    {busyDeleteId === rec.id ? 'Deleting…' : 'Delete'}
                  </button>
                </div>
              </div>
            );
