...
const MgrRequestDetail=({id,navigate,showToast})=>{const{data:r,loading,error,refresh}=useMgrRequestDetail(id);const[modal,setModal]=React.useState(null);const[busy,setBusy]=React.useState(false);const[actionError,setActionError]=React.useState(null);const runAction=async(op,label)=>{setBusy(true);setActionError(null);try{await op();showToast&&showToast(label);refresh();}catch(e){setActionError(e.message||String(e));}finally{setBusy(false);}};if(loading&&!r){return<Page title="Loading request…">
      ...
      </Page>;}const onApprove=()=>{if(!window.confirm(`Approve "${r.title}"?`))return;runAction(()=>api.requests.approve(r.id),`#${r.id} approved`);};const onMarkComplete=()=>{if(!window.confirm(`Mark "${r.title}" as complete? This closes the request.`))return;runAction(()=>api.requests.close(r.id),`#${r.id} closed`);};const onSubmitModal=async reason=>{const action=modal;setModal(null);if(action==='RETURN'){await runAction(()=>api.requests.returnRequest(r.id,reason),`#${r.id} returned`);}else if(action==='REJECT'){await runAction(()=>api.requests.reject(r.id,reason),`#${r.id} rejected`);}};const exps=(r.experiment_types||[]).map(et=>({id:et.id,name:et.name,code:et.name?et.name.split(/\s+/).map(t=>t[0]).join('').slice(0,4).toUpperCase():'—',group:'RA'}));const canAct=r.status==='submitted';const canComplete=r.status==='in_progress';return<Page breadcrumb={<Breadcrumb items={[{label:'All Requests',onClick:()=>navigate({page:'mgr_all_requests'})},{label:r.title}]}/>}title={r.title}subtitle={<span style={{display:'inline-flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
          <span style={{fontFamily:'var(--font-mono)',fontSize:12.5,color:mMuted,letterSpacing:'0.04em',padding:'3px 9px',borderRadius:6,background:'#ebebf0'}}>#{String(r.id).padStart(4,'0')}</span>
          <Pill kind={r.status}/>
        ...
        {canComplete&&<PrimaryBtn success disabled={busy}onClick={onMarkComplete}icon={<MI.Check size={14}/>}>{busy?'…':'Mark Complete'}</PrimaryBtn>}
      </>}>
      ...
      <Card padding={0}style={{marginBottom:18}}>
        ...
        <div style={{padding:'22px 24px',display:'grid',gridTemplateColumns:'repeat(4, 1fr)',gap:18,borderBottom:`1px solid ${mLineSft}`}}>
          {[{label:'Wafers',value:r.samples.length},{label:'Experiments',value:exps.length},{label:'Submitted',value:r.submitted?.split(' ')[0]||'—'},{label:'Requester',value:r.requester?.username||r.history[0]?.by||'—'}].map(s=><div key={s.label}>
        ...
        {r.note&&<div style={{padding:'16px 24px',borderBottom:`1px solid ${mLineSft}`}}>
        ...
        <div style={{padding:'18px 24px',display:'flex',flexDirection:'column',gap:14}}>
          {r.history.map((h,i)=>{const c={SUBMIT:{dot:'#5550a0',bg:'#e8e7f6',fg:'#5550a0'},APPROVE:{dot:'#157a4a',bg:'#c8eedd',fg:'#157a4a'},REJECT:{dot:'#c0394a',bg:'#fde4e4',fg:'#c0394a'},RETURN:{dot:'#a73d56',bg:'#f9d7e0',fg:'#a73d56'},CANCEL:{dot:'#777788',bg:'#ebebf0',fg:'#5a5a6e'}}[h.action]||{dot:'#a8a8b8',bg:'#f1f1f5',fg:'#5a5a6e'};return<div key={i}style={{display:'grid',gridTemplateColumns:'20px 1fr auto',gap:14,alignItems:'flex-start'}}>
                ...
                <div style={{minWidth:0}}>
                  ...
                  {h.note&&<div style={{fontSize:13,color:mText2,marginTop:6,padding:'8px 10px',background:mBgSoft,borderRadius:6}}>{h.note}</div>}
                </div>
      ...
      <Card padding={0}>
        ...
        <div style={{padding:18,display:'flex',flexDirection:'column',gap:10}}>
          {r.samples.map((s,si)=><button key={si}onClick={()=>navigate({page:'lab_wafer',id:s.id})}style={{display:'grid',gridTemplateColumns:'180px 1fr 20px',alignItems:'center',gap:18,padding:'14px 18px',background:'#fff',borderRadius:10,border:`1px solid ${mLine}`,textAlign:'left',cursor:'pointer',fontFamily:'inherit',transition:'border-color 0.12s, background 0.12s'}}onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(108,103,184,0.4)';e.currentTarget.style.background='#fbfbfd';}}onMouseLeave={e=>{e.currentTarget.style.borderColor=mLine;e.currentTarget.style.background='#fff';}}title={`Open wafer ${s.wafer}`}>
              <div>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
                {exps.map(e=><span key={e.id}style={{display:'inline-flex',alignItems:'center',gap:7,padding:'5px 11px 5px 6px',borderRadius:999,background:'#f5f5fa',border:`1px solid ${mLine}`}}>
      ...
      <ApprovalModal open={!!modal}action={modal}onClose={()=>setModal(null)}onSubmit={onSubmitModal}/>
    </Page>;};
