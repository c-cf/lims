...
import useMgrRequests from '@/components/Manager/hooks/useMgrRequests';
import ALL_REQ_TABS from '@/components/Manager/constants/allReqTabs';
...
const MgrAllRequests = ({ navigate }) => {
  ...
  const counts = React.useMemo(
    () =>
      Object.fromEntries(ALL_REQ_TABS.map((t) => [t.id, nonDraftRequests.filter(t.filter).length])),
    [nonDraftRequests],
  );
  const list = nonDraftRequests.filter(
    ALL_REQ_TABS.find((t) => t.id === tab)?.filter || (() => true),
  );
  ...
  return (
    ...
    >
      ...
      <div
        style={{ display: 'flex', gap: 22, borderBottom: `1px solid ${mLine}`, marginBottom: 22 }}
      >
        {ALL_REQ_TABS.map((t) => {
      ...
      <div style={{ fontSize: 13, color: mMuted, marginBottom: 14 }}>
        Showing <strong style={{ color: mInk }}>{list.length}</strong> of {nonDraftRequests.length}{' '}
        requests
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {list.length === 0 ? (
          <Card padding={48} style={{ textAlign: 'center', color: mMuted }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: mText2 }}>
          ...
          list.map((r) => {
            ...
            return (
              <button
                key={r.id}
                onClick={() => navigate({ page: 'mgr_request', id: r.id })}
                style={{
                  display: 'grid',
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  >
                    ...
                    <span style={{ fontFamily: 'var(--font-mono)' }}>
                      {(r.submitted || r.created || '').split(' ')[0] || '—'}
                    </span>
                    <span aria-hidden>·</span>
                    <span>
                      {sampleCount} wafer{sampleCount === 1 ? '' : 's'}
                    </span>
                    <span aria-hidden>·</span>
                    <span>
                      by{' '}
                      <span style={{ fontFamily: 'var(--font-mono)', color: mText2 }}>
                        {requester}
                      </span>
                    </span>
                  </div>
