        ...
        {list.length===0?<Card padding={48}style={{textAlign:'center',color:mMuted}}>
            ...
            <div style={{fontSize:14,fontWeight:600,color:mText2}}>No requests in this view</div>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                {(r.experiment_types||[]).map(e=>{const cat=e.lab_category||e.labCategory||'';const isRA=cat==='RA';const code=e.name?e.name.split(/\s+/).map(t=>t[0]).join('').slice(0,4).toUpperCase():'—';return<span key={e.id}style={{display:'inline-flex',alignItems:'center',gap:6,padding:'4px 9px 4px 4px',borderRadius:999,background:'#f5f5fa',border:`1px solid ${mLine}`}}>
                    <span style={{fontSize:10,fontWeight:700,padding:'2px 6px',borderRadius:999,background:isRA?'#e8e7f6':'#d4eaf0',color:isRA?'#5550a0':'#2a7a91',letterSpacing:'0.05em'}}>{code}</span>
