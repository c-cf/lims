...
const store={
  ...
  clear(){
    ['lims.access',
    'lims.refresh',
    'lims.user'].forEach(k=>this.set(k,null));
  }
};
const ROLE_MAP={
  fab_user:'fab_user',
...
function normalizeWip(w){
  ...
  return{
    ...
    dispatchCount:typeof w.dispatch_count==='number'?w.dispatch_count:(w.dispatches||[]).length,
    samples,
    status:w.status,
    note:w.note,
...
const api={
  ...
  auth:{
    ...
    async logout(){
      const refresh=store.get('lims.refresh');
      try{
        if(refresh)await call('/auth/logout',{
          method:'POST',
          body:{
            refresh_token:refresh
          }
  ...
  requests:{
    ...
    async submit(id){
      return normalizeRequestDetail(await call(`/requests/${id}/submit`,{
        method:'POST'
      }));
    },
    async approve(id){
      return normalizeRequestDetail(await call(`/requests/${id}/approve`,{
        method:'POST'
      }));
    },
    ...
    async ship(id){
      return normalizeRequestDetail(await call(`/requests/${id}/ship`,{
        method:'POST'
      }));
    },
    ...
    async close(id){
      return normalizeRequestDetail(await call(`/requests/${id}/close`,{
        method:'POST'
      }));
    },
  ...
  samples:{
    ...
    async receive(id){
      return normalizeSampleRow(await call(`/samples/${id}/receive`,{
        method:'POST'
      }));
    },
    ...
    async reportLost(id){
      return normalizeSampleRow(await call(`/samples/${id}/report-lost`,{
        method:'POST'
      }));
    },
    async void(id){
      return normalizeSampleRow(await call(`/samples/${id}/void`,{
        method:'POST'
      }));
    },
    async return(id){
      return normalizeSampleRow(await call(`/samples/${id}/return`,{
        method:'POST'
      }));
    }
  },
  wips:{
    ...
    async complete(id){
      return normalizeWip(await call(`/wips/${id}/complete`,{
        method:'POST'
      }));
    },
    async abort(id){
      return normalizeWip(await call(`/wips/${id}/abort`,{
        method:'POST'
      }));
    }
  },
  dispatches:{
    ...
    async start(id){
      return normalizeDispatch(await call(`/dispatches/${id}/start`,{
        method:'POST'
      }));
    },
    async unload(id){
      return normalizeDispatch(await call(`/dispatches/${id}/unload`,{
        method:'POST'
      }));
    },
    async recordResult(id,{
      comment=''
    }
    ={}){
    ...
    async complete(id){
      return normalizeDispatch(await call(`/dispatches/${id}/complete`,{
        method:'POST'
      }));
    },
    ...
    async redispatch(id){
      return normalizeDispatch(await call(`/dispatches/${id}/redispatch`,{
        method:'POST'
      }));
    },
    async abort(id){
      return normalizeDispatch(await call(`/dispatches/${id}/abort`,{
        method:'POST'
      }));
    }
  },
