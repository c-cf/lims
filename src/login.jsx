...
const LoginPage = ({ onLogin }) => {
  ...
  // Real auth: hit POST /auth/login via window.api (src/api.js). If api isn't
  // loaded (e.g. the standalone single-file artifact), fall back to the local
  // ACCOUNTS map so the demo still runs offline.
  const submit = async (e) => {
    ...
    // Offline fallback — demo only, used by the standalone single-file build.
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {Object.entries(ACCOUNTS).map(([u, a]) => (
                <button key={u} type="button" onClick={() => fillDemo(u)} style={{
                ...
                >
                  <div style={{
                    ...
                    background: u === 'fab_user' ? 'var(--tweak-fab-bg, linear-gradient(135deg, #f4a8bf, #bbb7e8))' : 'linear-gradient(135deg, #f4a8bf, #bbb7e8)',
